...
func NewPostgresWhatsGateSettingsRepository(pool *pgxpool.Pool) interfaces.WhatsGateSettingsRepository {
...
func (r *PostgresWhatsGateSettingsRepository) Get(ctx context.Context) (*entities.WhatsGateSettings, error) {
	row := r.pool.QueryRow(ctx, `SELECT id, whatsapp_id, api_key, base_url, created_at, updated_at FROM whatsgate_settings ORDER BY id DESC LIMIT 1`)
	var s entities.WhatsGateSettings
	if err := row.Scan(&s.ID, &s.WhatsappID, &s.APIKey, &s.BaseURL, &s.CreatedAt, &s.UpdatedAt); err != nil {
		return nil, err
	}
	return &s, nil
}
...
func (r *PostgresWhatsGateSettingsRepository) Save(ctx context.Context, s *entities.WhatsGateSettings) error {
	cmd := `INSERT INTO whatsgate_settings (whatsapp_id, api_key, base_url) VALUES ($1,$2,$3)
            ON CONFLICT (id) DO UPDATE SET whatsapp_id = EXCLUDED.whatsapp_id, api_key = EXCLUDED.api_key, base_url = EXCLUDED.base_url, updated_at = now()`
