...
func (m *MockBulkCampaignStorage) GetActiveCampaigns() ([]*domain.BulkCampaign, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	var result []*domain.BulkCampaign
	for _, campaign := range m.Campaigns {
		if campaign.Status == domain.CampaignStatusPending || campaign.Status == domain.CampaignStatusStarted {
			result = append(result, campaign)
		}
	}
	return result, nil
...
func (m *MockBulkCampaignStorage) IncrementErrorCount(id string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	if campaign, exists := m.Campaigns[id]; exists {
		campaign.ErrorCount++
